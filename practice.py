...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    # The solution below finds the smallest number without using built-in
    # function .sort()
...
def largest_int(numbers):
    ...
    **DO NOT USE** the built-in function `max()`!
    ...
    # The solution below finds the largest number without using built-in
    # function .sort()
...
def average(numbers):
    ...
    try:
        # Using the function sum_numbers defined on Line 169:
        return sum_numbers(numbers)/float(len(numbers))

    except ZeroDivisionError:
        return "Inputed list is empty."
...
def join_strings_with_comma(words):
    ...
    return joined_string.rstrip(", ")

    # The line below uses pythong build-in function
    #return ', '.join(words)
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    num_of_swaps = len(items) / 2

    for i in range(num_of_swaps):
        original_num = items[i]
        swap_number = items[-(i + 1)]
        items[i] = swap_number
        items[-(i+1)] = original_num

    # I believe this might be another solution:
    # items[::] = items[::-1]
...
def duplicates(items):
    ...
    for item in items:
        ...
        elif item not in dublicate_words:
            dublicate_words.append(item)

    dublicate_words.sort()
    return dublicate_words
